...
class Api::TourOperations::OrderOperations::Create < ApplicationOperation # rubocop:disable Metrics/ClassLength
  attr_reader :tour, :user, :customer_id
  ...
  def call # rubocop:disable Metrics/MethodLength, Metrics/AbcSize
    ActiveRecord::Base.transaction do
      ...
      total_tour_bus_seat_map = total_tour_bus_seat_map(tour_order, tour_bus_info)
      ...
      if tour.type_locate == Tour::INDAY
        ...
        total_price_special_food = total_price_special_food(tour_order)
        ...
        total_price_option = total_price_option(tour_order)
      ...
      description = "user: #{user.phone_number} purchased tour: #{tour.code}"
      if customer_id && tour_order_params[:payment_method] == Tour::Order::CREDIT_CARD
        order_status = payment_tour(customer_id, tour_order.total.round, description,
                                    params[:card_id])
        update_status_tour_order!(tour_order, order_status)
      end
      update_tour_user_point(user, tour_order)
  ...
  def create_tour_order!
    TourContracts::OrderContracts::Create.new(
      tour_order_params.merge(
        tour_id: tour.id,
        user_id: user.id,
        point:   user.point.to_i + user.point_bonus.to_i,
      ),
    ).valid!
    Tour::Order.create!(tour_order_params.merge!(
      tour_id:  tour.id,
      user_id:  user.id,
      order_no: generate_code(Tour::Order.name),
    ).except(:card_id))
  end
  ...
  def create_order_accompany(tour_order, tour_bus_info)
    ...
    params[:tour_order_accompanies].each do |order_accompany|
      ...
      if order_accompany[:is_owner] == false && order_accompany[:is_save] == true
  ...
  def create_user_contact(order_accompany)
    UserContactContracts::Create.new(order_accompany.permit!).valid!
  ...
  def create_tour_order_log_inday(tour_order)
    ...
    ).merge!(tour_order_seat_price(tour_order.tour))
  ...
  def create_tour_order_log_stay(tour_order)
    ...
    tour_order.create_tour_order_log!(
      ...
      tour_cancellation_patterns: {},
      tour_options:               price_option(tour_order),
      price_special_food:         {},
  ...
  def update_status_tour_order!(tour_order, order_status)
    tour_order.update!(
  ...
  def tour_order_params
    params.permit(
      ...
      :departure_date, :departure_start_location, :discount_amount, :invoice_desc, :is_seats_bus_free,
      :memo, :number_people, :payment_method, :payment_note, :payment_status, :seat_selection,
      :status, :used_points, :tour_bus_info_id, :tour_stay_departure_id, :card_id, :depature_time,
      :concentrate_time, :estimate_payment_amount, :estimate_payment_confirmation_date,
      :payment_confirmation_date, room: {}
    )
  end
  ...
  def tour_order_accompany_params
    params.permit(tour_order_accompanies: %i[
                    ...
                    selected_seat telephone tour_option_id tour_special_food_id is_user name_option
                  ])
  end
